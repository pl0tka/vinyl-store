...
export class CreateTables1730576859633 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(
            `CREATE TABLE \`orders\` (\`id\` int NOT NULL AUTO_INCREMENT, \`orderDate\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`price\` decimal(10,2) NOT NULL, \`status\` enum ('pending', 'success', 'failed') NOT NULL DEFAULT 'pending', \`userId\` varchar(255) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`
        );
        await queryRunner.query(
            `CREATE TABLE \`token_blacklist\` (\`id\` int NOT NULL AUTO_INCREMENT, \`token\` varchar(255) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`
        );
        await queryRunner.query(
            `CREATE TABLE \`change_logs\` (\`id\` int NOT NULL AUTO_INCREMENT, \`actionType\` enum ('create', 'update', 'delete') NOT NULL, \`entity\` varchar(255) NOT NULL, \`referenceId\` varchar(255) NOT NULL, \`newData\` json NULL, \`changeDate\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`
        );
        await queryRunner.query(
        ...
        await queryRunner.query(
            `ALTER TABLE \`orders_vinyls_vinyls\` ADD CONSTRAINT \`FK_eab4825adee117f8e85dbdfdb3f\` FOREIGN KEY (\`vinylsId\`) REFERENCES \`vinyls\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`
        );
    }
